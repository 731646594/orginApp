...
import {RFIDSpecScanListPage} from "../RFIDSpecScanList/RFIDSpecScanList";
...
@Component({
  selector: 'page-RFIDSpecScan',
  templateUrl: 'RFIDSpecScan.html'
})
export class RFIDSpecScanPage{
  pageItem = [
    ...
    this.storageService.getUserTable().executeSql(this.storageService.getSSS("willPlanDetail",this.userCode),[]).then(res=>{
      ...
      this.storageService.getUserTable().executeSql(this.storageService.getSSS("existPlanDetail",this.userCode),[]).then(res=>{
        if (res.rows.length>0){
          ...
          this.allPlan = [];
          this.allPlan = this.allPlan.concat(this.willPlan).concat(this.existPlan);
          this.numList["all"] = this.numList["will"]+this.numList["exist"];
          let len = 0;
          for (let i in this.scanList){
            len++
          }
  ...
  nextPage(value){
    this.app.getRootNav().push(RFIDSpecScanListPage,{type:value,data:this.scanList});
  }
  beginScan(){
    this.startScan();
    this.drawChart();
